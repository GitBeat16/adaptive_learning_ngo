...
def show_discovery():
    ...
    if st.button("Initiate Scan"):
        ...
        if peer:
            ...
            run_query("UPDATE profiles SET status='confirming', match_id=?, accepted=0 WHERE user_id=?", (m_id, peer[0]), commit=True)
...
def show_confirmation():
    ...
    my_acc = status_data[0] if status_data else 0
    peer_acc = peer_data[0] if peer_data else 0
...
def show_rating():
    ...
    if st.button("Submit Review"):
        run_query("INSERT INTO session_ratings (match_id, rater_id, rating) VALUES (?,?,?)",
                 (st.session_state.current_match_id, st.session_state.user_id, rating), commit=True)
        
        # Generate Quiz
        msgs = run_query("SELECT message FROM messages WHERE match_id=?", (st.session_state.current_match_id,), fetchall=True)
        transcript = " ".join([m[0] for m in msgs])
...
def show_quiz():
    ...
    if not quiz:
        st.write("No data available for quiz.")
    else:
...
def matchmaking_page():
    ...
    if res and res[0] == 'confirming' and st.session_state.session_step == "discovery":
        peer = run_query("SELECT a.name, p.user_id FROM profiles p JOIN auth_users a ON a.id=p.user_id WHERE p.match_id=? AND p.user_id!=?", (res[1], st.session_state.user_id), fetchone=True)
        if peer:
            st.session_state.peer_info = {"id": peer[1], "name": peer[0]}
